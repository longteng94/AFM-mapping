...
class JPKAnalyze(JPKRead):
    ...
    def get_snapin_distance(self, dirpath, *args, **kwargs):    
        ...
        defl_data = self.decode_data('vDeflection', segment_header_dict,
                                     extend_dir)['distance']
        #tip sample distance
        distance_data = (height_data + (defl_data)*48/39) # tip position (sensitivity correction)
        #print(force_data,height_data)
        tolerence = 0.01 #lower value mean wider range
        id_range_max = int(tolerence*len(distance_data)) #change
        #calculate snapin distance
        force_sobel = ndimage.sobel(force_data[id_range_max:]) #sobel transform
#        idx_min = np.argmin(force_sobel)
        #idx_max = np.argmax(force_sobel[id_range_max:])+id_range_max+1 # minimum force point
        #print(idx_max, id_range_max)
        idx_min = np.argmin(force_sobel)+id_range_max # jumpin point
        print(idx_min, id_range_max)
...
#            idx_min = len(force_data)-1
        print (distance_data[idx_min], distance_data[0], 
               distance_data[id_range_max], distance_data[-1])
#       print(distance_data[idx_min-1], distance_data[idx_min], distance_data[0], distance_data[-1])
        snapin_distance = distance_data[idx_min] - distance_data[-1] # for short distance
 #       snapin_distance = distance_data[idx_min] - distance_data[-1] # for long distance
        #TODO: define as fraction of extend distance range
        total_distance = distance_data[0] - distance_data[-1]
        print (snapin_distance)
        #tolerence = 0.8
        # The range (0,1), 0.8, 0.1
        snapin_distance = 0 if snapin_distance >= (1-tolerence) * total_distance \
                          else snapin_distance #change
        snapin_distance = 0 if snapin_distance <0 else snapin_distance
